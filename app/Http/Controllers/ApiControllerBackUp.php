...
class ApiControllerBackUp extends Controller
{
    ...
    public function get_jalur_terdekat_cepat_new($lat = -6.924318036348017 , $lon = 107.60681390762329)
    {
      ...
      ini_set('max_execution_time', 180);
        ...
        // perbaiki nearest ini, jangan pakai * biar bisa dirubah pake as shape_pt_lat, shape_pt_lon nya.
       // $nearest = DB::select("SELECT *, ( 6371 * acos( cos( radians(".$origin[0].") ) * cos( radians( shape_pt_lat ) ) * cos( radians( shape_pt_lon ) - radians(".$origin[1].") ) + sin( radians(".$origin[0].") ) * sin( radians( shape_pt_lat ) ) ) ) AS distance FROM shapes  HAVING distance < 2 ORDER BY distance  ;"); // limitnya di hilangkan dulu. LIMIT 0 , 20
       $nearest = DB::select("SELECT id,shape_pt_lat as lat, shape_pt_lon as lng, shape_id, shape_pt_sequence, place_info, ( 6371 * acos( cos( radians(".$origin[0].") ) * cos( radians( shape_pt_lat ) ) * cos( radians( shape_pt_lon ) - radians(".$origin[1].") ) + sin( radians(".$origin[0].") ) * sin( radians( shape_pt_lat ) ) ) ) AS distance FROM shapes  HAVING distance < 0.5 ORDER BY distance  ;");
        //filtering
        $data = [];
    ...
    { 
      ...
      { 
        ...
        $cetak_jalur2 = $this->cetak_jalur2_api($start, $finish, $walk_route);
        return $cetak_jalur2;
      }
      else
      {
